...
def measurement_loop(n):
    ...
    for i in range(n):
        call(["python", "exp_NPQ_algae.py", "with", "Daheng"])
...
def activation_and_rest(activation_time_HL = 240,
    ...
    # 2H ACTIVATION   
    print("ACTIVATION") 
    call(["python", "exp_TREATMENT.py", "with", "exp_duration=%d"%activation_time_HL, "arduino_LED.blue_param.analog_value=%d"%level_HL, "Daheng"])    
    print("RELAX LOW LIGHT")
    call(["python", "apply_constant_light.py", "with", "light_time=%d"%rest_time_LL,  "arduino_LED.blue_param.analog_value=%d"%level_LL, "Daheng"])
